...
def get_data_from_meta():
    ...
    # Dict[code] = [area]
    # stores a list of areas for each code in a dataset
    label_check = {}

    # iterate over line
    for line in origin_txt[1:]:
        # unpack line, split by delimiter
        # line => [y1, y2, y, domain, area, keywords, abstract]
        line = line.rstrip('\n')
        line = line.split('\t')
        assert len(line) == 7  # ensure line is correct length

        # get sample label from line
        sample_label = [
            line[3].rstrip().lstrip(),  # domain
            line[4].rstrip().lstrip()  # area
        ]
        # code = [y1]-[y2]
        code = str(line[0]) + '-' + str(line[1])

        # if code already in label_check
        if code in label_check.keys():
            # if area not in label_check[code]
            if sample_label[1] not in label_check[code]:
                label_check[code].append(sample_label[1])
        # if code not in label_check
        else:
            # add code to label_check as [area]
        ...
        doc = clean_str(doc)
        sample_text = doc
        # append total length to list, accumulating statistics
        total_len.append(len(sample_text))
        # append sample_text and sample_label
